    ...
    if search_response.status_code == 200:
        ...
        if table:
            ...
            if not df.empty:
                ...
                df_transposed = df.melt(id_vars=['Narration'], var_name='Year', value_name='Value')
                df_pivoted = df_transposed.pivot(index='Year', columns='Narration', values='Value')

                # Reset the index to make 'Year' a column
                df_pivoted.reset_index(inplace=True)

                # Print the DataFrame
                print(df_pivoted.head(16))
                df_pivoted.to_csv('reliance_balance_sheet_transposed.csv', index=False)
        else:
